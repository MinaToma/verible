...
for i in "$@"; do
  ...
  bazel-bin/verilog/tools/kythe/verible-verilog-kythe-extractor --file_list_path "$(dirname $i)/file_list.txt" --printextraction --file_list_root "$(dirname $i)"  --print_kythe_facts json  > "${KYTHE_OUT}"/entries
